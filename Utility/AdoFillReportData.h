...
inline _bstr_t BuildDBFieldName(LPOLESTR FieldName, int col)
{
...
inline BOOL FetchRecordFromAdoQuery(IGridppReport *pReport, CString &ConnectionString, CString &QuerySQL)
{
    HRESULT  hr;
    ...
    hr = pConnection->Open(bstrConnectionString, L"", L"", adConnectUnspecified); //hr = pConnection->Open(m_ConnectionString, L"", L"", adConnectUnspecified);
	if ( !SUCCEEDED(hr) )
		return FALSE;
	...
	for (int i=0; i<Count; i++)
	{
		...
		if ( FieldName )
		{
			...
			IGRField *pField = pReport->FieldByName( bstrFieldName );
			if ( pField )
			{
				ADOFIELD_PAIR fp;
